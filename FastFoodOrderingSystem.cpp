// Filename: FastFoodOrderingSystem.cpp
...
public:
    ...
    float calculateDiscountedTotal() const {
        float total = calculateTotal();
        if (total >= 1000) return total * 0.9; // 10% discount
        return total;
    }
    ...
    void displayOrder() const {
        ...
        float total = calculateTotal();
        float discounted = calculateDiscountedTotal();
        cout << fixed << setprecision(2);
        cout << "\nSubtotal: Rs." << total << endl;
        if (discounted < total)
            cout << "Discount Applied: Rs." << total - discounted << endl;
        cout << "Total Payable: Rs." << discounted << endl;
    }

    void processPayment() const {
        float total = calculateDiscountedTotal();
        cout << fixed << setprecision(2);
    ...
    void saveToFile(const string& filename) const {
        ...
        outFile << "Total: Rs." << calculateDiscountedTotal() << "\n";
        outFile << "---------------------------------\n\n";
        outFile.close();
    }
};
...
public:
    Menu() {
        ...
        sandwichMenu[3] = make_unique<Sandwich>("Extreme Veg Cheese Sandwich", 100);
